...
import { booleanAttribute, Component, computed, effect, inject, input, PLATFORM_ID } from '@angular/core';
...
type KeyOfPresets = keyof typeof presets;
...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, FormsModule, SelectButton, Drawer, RadioButton],
    template: `
        <p-drawer [visible]="visible()" (onHide)="onDrawerHide()" position="right" header="Settings" styleClass="layout-config-sidebar w-80">
            <div class="flex flex-col gap-6">
                ...
                <div>
                    ...
                    <div class="pt-2 flex gap-2 flex-wrap">
                        @for (surface of surfaces; track surface.name) {
                            <button type="button" (click)="updateColors('surface', surface)" class="w-6 h-6 cursor-pointer hover:shadow-lg rounded duration-150 flex items-center justify-center" [style.backgroundColor]="surface.palette['500']">
                                @if (selectedSurface() ? selectedSurface() === surface.name : isDarkTheme() ? surface.name === 'zinc' : surface.name === 'slate') {
                                    <i class="pi pi-check text-white"></i>
                                }
                            </button>
                        }
                ...
                <div>
                    <div class="flex flex-col gap-2">
                        ...
                        <p-selectbutton [ngModel]="selectedPreset()" (ngModelChange)="onPresetChange($event)" [options]="presetOptions" [allowEmpty]="false" />
                    </div>
                </div>
                ...
                <div>
                    <div class="flex flex-col gap-2">
                        ...
                        <p-selectbutton [ngModel]="isDarkTheme()" (ngModelChange)="toggleDarkMode()" [options]="themeOptions" optionLabel="name" optionValue="value" [allowEmpty]="false" />
                    </div>
                </div>
                            ...
                            <div class="flex flex-wrap flex-col gap-3">
                                <div class="flex">
                                    <div class="flex items-center gap-2 w-6/12">
                                        <p-radiobutton name="menuMode" value="static" [ngModel]="menuMode()" (ngModelChange)="changeMenuMode($event)" inputId="static" />
                                        <label for="static">Static</label>
                                    </div>
                                    <div class="flex items-center gap-2 w-6/12">
                                        <p-radiobutton name="menuMode" value="overlay" [ngModel]="menuMode()" (ngModelChange)="changeMenuMode($event)" inputId="overlay" />
                                        <label for="overlay">Overlay</label>
                                    </div>
                                </div>
                                <div class="flex">
                                    <div class="flex items-center gap-2 w-6/12">
                                        <p-radiobutton name="menuMode" value="slim" [ngModel]="menuMode()" (ngModelChange)="changeMenuMode($event)" inputId="slim" />
                                        <label for="slim">Slim</label>
                                    </div>
                                    <div class="flex items-center gap-2 w-6/12">
                                        <p-radiobutton name="menuMode" value="compact" [ngModel]="menuMode()" (ngModelChange)="changeMenuMode($event)" inputId="compact" />
                                        <label for="compact">Compact</label>
                                    </div>
                                </div>
                                <div class="flex">
                                    <div class="flex items-center gap-2 w-6/12">
                                        <p-radiobutton name="menuMode" value="reveal" [ngModel]="menuMode()" (ngModelChange)="changeMenuMode($event)" inputId="reveal" />
                                        <label for="reveal">Reveal</label>
                                    </div>
                                    <div class="flex items-center gap-2 w-6/12">
                                        <p-radiobutton name="menuMode" value="drawer" [ngModel]="menuMode()" (ngModelChange)="changeMenuMode($event)" inputId="drawer" />
                                        <label for="drawer">Drawer</label>
                                    </div>
                                </div>
                                <div class="flex">
                                    <div class="flex items-center gap-2 w-6/12">
                                        <p-radiobutton name="menuMode" value="horizontal" [ngModel]="menuMode()" (ngModelChange)="changeMenuMode($event)" inputId="horizontal" />
...
export class AppConfigurator {
    ...
    primaryColors: PrimaryColor[] = [
        ...
        { name: 'cyan', palette: { 50: '#ecfeff', 100: '#cffafe', 200: '#a5f3fc', 300: '#67e8f9', 400: '#22d3ee', 500: '#5bc0de', 600: '#4ab0c9', 700: '#3a90a5', 800: '#2a7080', 900: '#1a505b', 950: '#0d282e' } },
    ...
    updateColors(type: 'primary' | 'surface', color: PrimaryColor | Surface) {
        if (type === 'primary') {
    ...
    applyTheme(type: 'primary' | 'surface', color: PrimaryColor | Surface) {
        if (type === 'primary') {
            updatePreset(this.getPresetExt());
        } else if (type === 'surface') {
            updateSurfacePalette((color as Surface).palette);
        }
    }
    ...
    onPresetChange(presetName: string) {
        ...
        const preset = presets[presetName as KeyOfPresets];
    ...
    changeMenuMode(mode: string) {
        this.layoutService.changeMenuMode(mode);
    }
